...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  const performAutoDetection = async () => {
    ...
    if (typeof navigator !== 'undefined' && navigator.platform?.toLowerCase().includes('mac')) {
      const homebrewDbPath = '/usr/local/var/siftnotes/meeting_minutes.db';
      try {
  ...
  const verifyModelStatus = async (savedStatus: OnboardingStatus) => {
    ...
    // API-key flow: Step 1: Welcome, Step 2: API keys, Step 3: Permissions (macOS)
    let currentStep = savedStatus.current_step;
    let completed = savedStatus.completed;

    // Clamp step to current max.
    if (currentStep > 3) {
      currentStep = 3;
    }

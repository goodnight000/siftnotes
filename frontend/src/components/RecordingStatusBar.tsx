...
export const RecordingStatusBar: React.FC<RecordingStatusBarProps> = ({ isPaused = false }) => {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-2 px-3 py-2 bg-gray-50 rounded-lg mb-2 min-w-0 max-w-full"
    >
      <div className={`w-2 h-2 shrink-0 rounded-full ${isPaused ? 'bg-orange-500' : 'bg-red-500 animate-pulse'}`} />
      <span className={`text-sm min-w-0 truncate ${isPaused ? 'text-orange-700' : 'text-gray-700'}`}>

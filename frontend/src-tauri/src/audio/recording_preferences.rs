...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\siftnotes-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("siftnotes-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("siftnotes-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/siftnotes-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("siftnotes-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/siftnotes-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("siftnotes-recordings")
    }

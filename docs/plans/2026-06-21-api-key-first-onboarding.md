...
**Goal:** Make SiftNotes first-run setup default to user-owned API keys while preserving a local-only fallback.

**Architecture:** Add an onboarding setup mode to the React onboarding context. API-key mode saves provider settings directly and completes onboarding through a new backend command that only marks onboarding complete. Local mode keeps the existing local model download path.
...
Step 2 renders the new API-key setup step. Step 3 renders local downloads only when setup mode is local; otherwise it renders permissions on macOS or completes on non-macOS.
...
- Transcription provider: Groq, OpenAI, Deepgram, Local.
- Transcription model/API key where needed.
